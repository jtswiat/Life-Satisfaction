...
dat$x64 <- ifelse(is.na(dat$v64), 1, 0)
dat$v64[is.na(dat$v64)] <- median(dat$v64, na.rm = TRUE)
...
dat$x69 <- ifelse(is.na(dat$v69), 1, 0)
dat$v69[is.na(dat$v69)] <- 0
...
dat$v100[is.na(dat$v100)] <- median(dat$v100, na.rm = TRUE)
...
dat$c120 <- ifelse(is.na(dat$v120), 1, 0)
dat$v120 <- median(dat$v120, na.rm = TRUE)
dat$v121 <- as.numeric(as.character(dat$v121))
dat$c121 <- ifelse(is.na(dat$v121), 1, 0)
dat$v121 <- median(dat$v121, na.rm = TRUE)
...
dat$DayOfWeek[is.na(dat$DayOfWeek)] <- "Wednesday"
...
dat$c130 <- ifelse(is.na(dat$v130), 1, 0)
dat$v130[is.na(dat$v130)] <- median(dat$v130, na.rm = TRUE)
dat$v132 <- as.numeric(dat$v132)
dat$c132 <- ifelse(is.na(dat$v132), 1, 0)
dat$v132[is.na(dat$v132)] <- median(dat$v132, na.rm = TRUE)
...
dat$c153 <- ifelse(is.na(dat$v153), 1, 0)
dat$v153[is.na(dat$v153)] <- 1900
...
dat$c168[is.na(dat$c168)] <- "M"
dat$c168 <- as.factor(dat$c168)
dat$c168 <- mapvalues(dat$c168, levels(dat$c168), c("A", "B", "C", "D", "M"))
dat$v168 <- as.numeric(as.character(dat$v168))
dat$v168[is.na(dat$v168)] <- 0
...
dat$v251[is.na(dat$v251)] <- 0
dat$v252 <- as.numeric(as.character(dat$v252))
dat$v252[is.na(dat$v252)] <- 0
...
# Bucket Belgium with GB due to low credibility
write.csv(dat, "testRF.csv")
```
